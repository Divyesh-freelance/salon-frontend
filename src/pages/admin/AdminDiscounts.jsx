...
const CATEGORIES = ['Hair', 'Facial', 'Makeup', 'Bridal', 'Spa', 'Nails', 'Skin Care']

function DiscountForm({ onSubmit, defaultValues, isLoading }) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input {...register('discountPercentage', { required: true, valueAsNumber: true, min: 1, max: 100 })} type="number" className="admin-input" placeholder="30" />
    ...
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="font-sans text-sm text-stone-500 mt-1">{discounts.length} offers · {discounts.filter(d => d.isActive).length} active</p>
        </div>
        <button
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <button
                      ...
                      className={`px-3 py-1 rounded-full font-sans text-xs font-semibold transition-all ${d.isActive ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-stone-100 text-stone-500 hover:bg-stone-200'}`}
                    >
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { setEditing(d); setModalOpen(true) }} className="flex-1 border border-stone-200 py-2 font-sans text-xs uppercase tracking-wider hover:border-stone-900 transition-all">Edit</button>
                    <button onClick={() => { if (confirm('Delete this offer?')) deleteMut.mutate(d.id) }} className="flex-1 border border-red-200 text-red-600 py-2 font-sans text-xs uppercase tracking-wider hover:bg-red-50 transition-all">Delete</button>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      {discounts.length === 0 && !isLoading && <p className="text-center font-sans text-stone-400 py-16">No offers yet.</p>}

      <Modal isOpen={modalOpen} onClose={() => { setModalOpen(false); setEditing(null) }} title={editing ? 'Edit Offer' : 'Create Offer'} size="lg">
        <DiscountForm
          onSubmit={(d) => editing ? updateMut.mutate({ id: editing.id, data: d }) : createMut.mutate(d)}
          defaultValues={editing || { isActive: true, trending: false, ctaText: 'Book Now', sortOrder: 0 }}
